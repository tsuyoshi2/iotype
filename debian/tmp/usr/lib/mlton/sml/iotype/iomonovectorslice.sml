...
functor IOMonoVectorSlice (
	...
	open VectorSlice
	val writeElem = Vector.writeElem
	val readElem = Vector.readElem
	fun writeSlice (p, s) =
		let
			val (vector, offset, length) = base s
		in
			Vector.writeVector (p, vector)
			; writeInt (p, offset)
			; writeInt (p, length)
		end
	fun readSlice p = case Vector.readVector p of
		NONE => NONE
